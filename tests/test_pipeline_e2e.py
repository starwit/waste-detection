...
def test_pipeline_end_to_end(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    # Training folder 1
    images_dir = tmp_path / "raw_data" / "train" / "source1" / "images"
    labels_dir = tmp_path / "raw_data" / "train" / "source1" / "labels"
    ...
    images_dir2 = tmp_path / "raw_data" / "train" / "source2" / "images"
    labels_dir2 = tmp_path / "raw_data" / "train" / "source2" / "labels"
    ...
    test_images_dir = tmp_path / "raw_data" / "test" / "sourceT" / "images"
    test_labels_dir = tmp_path / "raw_data" / "test" / "sourceT" / "labels"
    ...
    # create an instance for pipeline calls
    args = Args()
    ...
    # 1) Datasets created
    assert (tmp_path / "datasets" / args.dataset_name).exists()
    # 2) Results written by mean_table and include scene metric column
    results_csv = tmp_path / "results_comparison" / "results.csv"
    ...
    for row in rows:
        ...
        if val_str and val_str != "-":
            try:
                ...
                pytest.fail(f"{col_name} is not numeric: {val_str}")
